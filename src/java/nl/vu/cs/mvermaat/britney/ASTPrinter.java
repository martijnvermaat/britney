 ...
 * 7/9/2001
 *
 * Modified by Martijn Vermaat <mvermaat@cs.vu.nl>
 * (adapted to use in Britney)
 * 27/2/2004
 */
public class ASTPrinter extends ReversedDepthFirstAdapter
{
    ...
    public void outAApplicationTerm(AApplicationTerm node)
    {
		...
		output = indent + "- " + setColor(BOLD, FG_CYAN, BG_BLACK) +
			...
			(new String("@").lastIndexOf('.') + 1) + treeColor() +
			"\n" + output;
    ...
    public void outAAbstractionTerm(AAbstractionTerm node)
    {
		...
		// prepend this line to the output.
		output = indent + "- " + setColor(BOLD, FG_CYAN, BG_BLACK) +
			new String("\\" + node.getVariable().getText()).substring
			(new String("\\" + node.getVariable().getText()).lastIndexOf('.') + 1) + treeColor() +
			"\n" + output;
    ...
    public ASTPrinter(PrintWriter out) {
